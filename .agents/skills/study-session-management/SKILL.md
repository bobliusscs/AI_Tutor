...
**响应顺序要求（重要）**：
1. **立即**调用 `save_study_summary` 保存记录，`summary` 传空字符串
2. **同时**简短回复用户"学习记录已保存"（不超过15字，如"好的，学习记录已保存！"）
3. 不要等待摘要生成，前端会自动清空消息返回起始状态
4. 个性化摘要由前端在后台异步生成，模型无需生成复杂摘要

触发模式示例：
- 用户: "今天就到这里吧，感谢陪伴！"
- AI: **同时**调用 save_study_summary(summary="") 保存记录 → **立即**简短确认"好的，学习记录已保存！"
...
**模型思维**:
1. 用户表达了结束学习意图
2. **立即**调用 save_study_summary 保存会话记录，summary 传空
3. **同时**简短回复用户确认，不要让用户等待
4. 前端会自动清空消息返回起始状态，并在后台异步生成个性化摘要
...
**第一轮 - 立即简短回复**:
"好的，学习记录已保存！" 或 "学习记录已保存，下次见！"（不超过15字）
...
2. **简短回复优先**: 用户表达结束意图时，**第一时间**调用工具保存记录，同时输出简短确认（不超过15字）。不要让用户等待摘要生成
3. **摘要由前端异步生成**: `summary` 字段非必需，前端会在后台调用 LLM 生成个性化摘要并保存到数据库。模型无需在 tool_call 中生成复杂摘要
4. **前端自动清空状态**: 工具返回成功后，前端会自动清空消息并返回起始状态。模型第二轮无需输出长文本或重复告别
5. **历史记录作为引导**: get_recent_sessions_summary 返回的摘要应该作为模型回复的参考，帮助模型给出更连贯的学习引导
6. **conversation_log 格式**: 必须是JSON数组格式，包含完整的对话记录，每条消息包含 role 和 content 字段